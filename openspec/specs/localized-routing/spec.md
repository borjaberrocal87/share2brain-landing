...
Serve each supported language of the landing page from its own pre-rendered static URL (Spanish at `/`, English at `/en/`) so that localized content is fully crawlable without JavaScript, language selection is a navigation between locale URLs rather than a runtime toggle, and each locale page emits reciprocal localization metadata (hreflang, canonical, og:locale) and is enumerated in a sitemap.
