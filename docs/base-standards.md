---
description: Core development rules and guidelines for the KeepHive Landing project, applicable to all AI agents.
alwaysApply: true
---

# KeepHive Landing — Base Standards

## 1. Project Overview

**KeepHive** is an open-source AI agent for Discord communities that auto-indexes channel messages and answers questions with verifiable citations (RAG). It is self-hosted and configured via YAML.

**KeepHive Landing** (`keephive-landing`) is the marketing landing page and documentation site for KeepHive. It is a **static site** (SSG) — not a web application. It has no backend, no database, and no authentication. Its purpose is to:

- Explain what KeepHive is and what problem it solves
- Show how it works (architecture diagram)
- Provide a quick-start installation guide
- Link to the full documentation and GitHub repository

The landing is deployed to **Cloudflare Pages** (or Vercel) and served from a global CDN.

### Domain Language

| Term | Definition |
|------|-----------|
| **KeepHive** | The open-source AI agent for Discord communities. |
| **Self-hosted** | KeepHive runs on the operator's own infrastructure via Docker Compose. |
| **RAG** | Retrieval-Augmented Generation — answering questions using indexed context with citations. |
| **Indexing** | The process of reading Discord channel messages and storing embeddings in pgvector. |
| **pgvector** | PostgreSQL extension used for vector similarity search. |
| **Islands** | Astro's pattern for hydrating interactive React components on a static page. |
| **Landing** | This repository — the marketing site and docs. |

### Tech Stack

| Layer | Technology | Rationale |
|-------|-----------|-----------|
| **Framework** | **Astro 4+** | SSG with islands, native SEO, fast builds |
| **UI Components** | **React 18** (islands) | Interactive components only (ThemeToggle, CodeBlock, MobileMenu) |
| **Styling** | **Tailwind CSS 3+** | Utility-first, rapid development |
| **Typography** | **Inter** (body), **Space Grotesk** (headings), **JetBrains Mono** (code) | Legible, modern, free |
| **Icons** | **Lucide React** | Consistent, lightweight |
| **Animations** | **Framer Motion** | Smooth, performant scroll/transition animations |
| **Analytics** | **Plausible** or **Umami** | Privacy-friendly, GDPR compliant |
| **Hosting** | **Cloudflare Pages** | Global CDN, HTTPS, deploy previews, free |
| **Domain** | **keephive.dev** | Developer-focused TLD |
| **CI/CD** | **GitHub Actions** | Native repo integration |
| **Linting** | **ESLint + Prettier** | Code quality and formatting |

## 2. Core Principles

- **Small tasks, one at a time**: Always work in baby steps, one at a time. Never go forward more than one step.
- **Test-Driven Development**: Start with failing tests for any new functionality (TDD), according to the task details.
- **Type Safety**: All code must be fully typed (TypeScript strict mode).
- **Clear Naming**: Use clear, descriptive names for all variables and functions.
- **Incremental Changes**: Prefer incremental, focused changes over large, complex modifications.
- **Question Assumptions**: Always question assumptions and inferences.
- **Pattern Detection**: Detect and highlight repeated code patterns.

## 3. Language Standards

- **English Only**: All technical artifacts must always use English, including:
    - Code (variables, functions, classes, comments, error messages)
    - Documentation (README, guides, API docs)
    - Configuration files and scripts
    - Git commit messages
    - Test names and descriptions

## 4. Coding Conventions

- **Conventional Commits**: Use `feat:`, `fix:`, `chore:`, `docs:`, `refactor:`, `test:` prefixes.
- **Branch Naming**: `feature/<ticket-id>-short-description`, `fix/<ticket-id>-short-description`.
- **File Naming**:
    - Astro components: PascalCase (`Header.astro`, `Hero.astro`)
    - React islands: PascalCase with `.tsx` (`ThemeToggle.tsx`, `CodeBlock.tsx`)
    - Pages: kebab-case (`getting-started.md`, `changelog.astro`)
    - Utilities: camelCase (`formatDate.ts`)
    - Styles: `global.css`

## 5. Specific Standards

For detailed standards and guidelines specific to different areas of the project, refer to:

- [Frontend Standards](./frontend-standards.md) — Astro components, React islands, styling, SEO, performance
- [Build & Deploy Standards](./backend-standards.md) — Build pipeline, deployment, CI/CD, hosting configuration
- [Documentation Standards](./documentation-standards.md) — Technical documentation structure and maintenance
- [OpenSpec Tasks Mandatory Steps](./openspec-tasks-mandatory-steps.md) — Required checklist for OpenSpec `tasks.md` files

## 6. Project Skills

- Skills live in `ai-specs/skills`.
- When a request matches a skill, load and follow the corresponding `SKILL.md` automatically before continuing.
- Also load any referenced files in the skill folder (for example, `references/*.md`) when the skill requires them.

## 7. Planning Model Requirement

Planning workflows must run with Opus high reasoning.

This requirement applies to:
- `enrich-us`
- `openspec-ff-change`
- `openspec-continue-change`

Before starting any of these workflows, verify the session is using Opus high reasoning. If it is not, **self-correct** by adding `"model": "claude-opus-4-7"` to `.claude/settings.json` (use the `update-config` skill or edit directly), then continue — do not stop and ask the user. Do the same to come back to sonnet medium for any other step.

## 8. Symlink Integrity and Multi-Agent Portability

- **Canonical Source**: Keep reusable artifacts in `ai-specs` as the canonical source. Agent-specific paths (such as `.claude` and `.cursor`) should reference them through symlinks when possible.
- **Update Safety**: Whenever a file is renamed, moved, or its suffix changes, verify and update all symlinks that target it before considering the change complete.
- **New Artifact Linking**: Whenever creating a new artifact that requires multi-agent exposure, create the corresponding symlinks from the expected agent-specific reference paths.
- **External Customization Review**: Whenever customization is introduced outside `ai-specs`, evaluate whether it should be moved into `ai-specs` and replaced with symlinks.
- **Completion Gate**: A change is incomplete if it leaves broken symlinks, stale targets, or duplicated canonical artifacts across agent-specific folders.

## 9. Mandatory OpenSpec Artifact Updates for Post-Apply Changes

When a new fix/change request appears after `opsx:apply` (or `/apply`) and before `opsx:archive` (or `/archive`), agents must treat it as a spec update first, not as an informal "fix this quickly". Documentation is the source of truth.

Required order:

1. Update the current OpenSpec change artifacts that are affected (for example: scenarios, requirements/specs, and `tasks.md`). Don't add tasks as "bugfixes" but as part of the initial design, thus in the proper section.
2. If artifact regeneration is needed, run the corresponding OpenSpec step (`opsx:continue`, `opsx:ff`, or equivalent) before coding.
3. Implement code only after artifacts reflect the new request.
4. Re-run verification against the updated artifacts before archiving.

Do not apply direct code-only fixes in this window without updating OpenSpec artifacts.
